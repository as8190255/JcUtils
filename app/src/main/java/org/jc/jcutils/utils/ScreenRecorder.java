 ...
 * 源码来源于:https://github.com/yrom/ScreenRecorder
 */
...
public class ScreenRecorder extends Thread {
    ...
    private int mDpi;
    private String mDstPath;//文件保存路径
    private MediaProjection mMediaProjection;//屏幕投射类
    ...
    private MediaCodec mEncoder;//编解码的核心类
    private Surface mSurface;//一块画布
    private MediaMuxer mMuxer;//多媒体合成器(可以写入文件) - 编码
    //MediaExtractor 读取视频数据用 - 解码
    private boolean mMuxerStarted = false;
    private int mVideoTrackIndex = -1;
    private AtomicBoolean mQuit = new AtomicBoolean(false);
    private MediaCodec.BufferInfo mBufferInfo = new MediaCodec.BufferInfo();//bytebuffer
    private VirtualDisplay mVirtualDisplay;//获取屏幕流类, 获取到屏幕信息后 渲染到Surface
    ...
    private void recordVirtualDisplay() {
        while (!mQuit.get()) {
            int index = mEncoder.dequeueOutputBuffer(mBufferInfo, TIMEOUT_US);//获取屏幕输出流,并且返回id
            Log.i(TAG, "dequeue output buffer index=" + index);

            //根据id的内容 判断进行不同的数据处理
            if (index == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {//格式发生了变化
                resetOutputFormat();//

            } else if (index == MediaCodec.INFO_TRY_AGAIN_LATER) {//等待10毫秒
...
//            }
    ...
    private void prepareEncoder() throws IOException {
        // 创建多媒体格式 => 设置编码格式为: H.264编码 ,设置视频宽高
        MediaFormat format = MediaFormat.createVideoFormat(MIME_TYPE, mWidth, mHeight);

        format.setInteger(MediaFormat.KEY_COLOR_FORMAT,
                MediaCodecInfo.CodecCapabilities.COLOR_FormatSurface);//surface进行mediaCodec编码

        format.setInteger(MediaFormat.KEY_BIT_RATE, mBitRate);//设置波特率
        format.setInteger(MediaFormat.KEY_FRAME_RATE, FRAME_RATE);//帧率数
        format.setInteger(MediaFormat.KEY_I_FRAME_INTERVAL, IFRAME_INTERVAL);//关键帧间隔 单位s秒
